...
async function _fillV2Template(wb, client, sorted, prices) {
  ...
  // ── New V2 template layout (30 rows, generated by Python/openpyxl) ──
  // R1 navy bar, R2 company, R3 cyan sep, R4 title, R5 labels, R6 values,
  // R7 sep, R8-R9 table header, R10-R26 data (17 slots),
  // R27 total count, R28 sep, R29 total payment, R30 footer
  var FIRST_DATA_ROW = 10, TEMPLATE_SLOTS = 17, ORIG_LAST_COL = 9;
  var FIRST_OP_COL = 2, LAST_OP_COL = 9, HEADER_ROW = 9;
  ...
  if (extraOps.length > 0) {
    ...
    // Write extra column headers in R9
    ...
    for (var ei = 0; ei < extraOps.length; ei++) {
      ...
      ws.getColumn(eCol).width = ws.getColumn(LAST_OP_COL).width || 11;
    }
    hRow.commit();

    // Extend styles for R1-R8 extra columns
    ...
    // Targeted merge extensions for new V2 layout
    // Full-width: R1(A1:I1), R3(A3:I3), R4(A4:I4), R7(A7:I7)
    [1, 3, 4, 7].forEach(function(r) {
      try { ws.unMergeCells(r, 1, r, ORIG_LAST_COL); } catch(e) {}
      try { ws.mergeCells(r, 1, r, LAST_COL); } catch(e) {}
    });
    // R8: B8:H8 -> B8:(LAST_COL-1) — I column is standalone
    try { ws.unMergeCells(8, 2, 8, 8); } catch(e) {}
    try { ws.mergeCells(8, 2, 8, LAST_COL - 1); } catch(e) {}

    // Footer merges
    // R28: A28:I28 -> A28:LAST_COL
    try { ws.unMergeCells(28, 1, 28, ORIG_LAST_COL); } catch(e) {}
    try { ws.mergeCells(28, 1, 28, LAST_COL); } catch(e) {}
    // R27: C27:I27 -> C27:LAST_COL
    try { ws.unMergeCells(27, 3, 27, ORIG_LAST_COL); } catch(e) {}
    try { ws.mergeCells(27, 3, 27, LAST_COL); } catch(e) {}
    // R29: G29:I29 -> G29:LAST_COL
    try { ws.unMergeCells(29, 7, 29, ORIG_LAST_COL); } catch(e) {}
    try { ws.mergeCells(29, 7, 29, LAST_COL); } catch(e) {}
    // R30: F30:I30 -> F30:LAST_COL
    try { ws.unMergeCells(30, 6, 30, ORIG_LAST_COL); } catch(e) {}
    try { ws.mergeCells(30, 6, 30, LAST_COL); } catch(e) {}

    // Fix outer borders: col I right:medium → thin, LAST_COL right:medium
    var medBrd = { style: 'medium' };
    var thinBrd = { style: 'thin' };
    for (var ofr = 1; ofr <= 30; ofr++) {
      var oRow = ws.getRow(ofr);
      var cellI = oRow.getCell(ORIG_LAST_COL);
      if (cellI.border) {
        var bi = JSON.parse(JSON.stringify(cellI.border));
        if (bi.right && bi.right.style === 'medium') { bi.right = thinBrd; cellI.border = bi; }
      }
      for (var imc = ORIG_LAST_COL + 1; imc < LAST_COL; imc++) {
        var imCell = oRow.getCell(imc);
        if (imCell.border) {
          var imb = JSON.parse(JSON.stringify(imCell.border));
          if (imb.right && imb.right.style === 'medium') { imb.right = thinBrd; imCell.border = imb; }
        }
      }
  ...
  // ── 3. Fill header values (R6) ──
  // New template: A6:B6, C6:E6, F6:G6, H6 (standalone — no I6 merge)
  _setCellValue(ws, 6, 1, client.name || '');
  _setCellValue(ws, 6, 3, period);
  _setCellValue(ws, 6, 6, docNr);
  _setCellValue(ws, 6, 8, todayStr);

  // ── 4. Write data rows (R10-R26, preserve template styles) ──
  var checkFont = { name: 'Arial', size: 12, bold: true, color: { argb: 'FF1A6B2A' } };
  var dataFont  = { name: 'Arial', size: 9, color: { argb: 'FF0D2D5A' } };
  ...
  // ── 6. Write formulas (new layout: R27 count, R29 payment, R30 footer) ──
  // R27: "Total interventii efectuate" (A27) + COUNTA formula in B27
  // Template text in A27 is kept, only update formula in B27
  _setCellFormula(ws, 27, 2, 'COUNTA(A' + FIRST_DATA_ROW + ':A' + lastDataRow + ')');

  // R29: "TOTAL DE PLATA (RON)" (A29:F29) + amount in G29:I29
  // Template text kept, only update value in G29
  _setCellFormula(ws, 29, 7, 'IFERROR(COUNTA(A' + FIRST_DATA_ROW + ':A' + lastDataRow + ')*' + pretIntv + ',0)');

  // R30: footer — keep template text as-is
...
async function _fillV1Template(wb, client, sorted, prices) {
  ...
  // ── 1. Data font (matches new Python-generated template) ──
  var dataFont = { name: 'Arial', size: 9, color: { argb: 'FF0D2D5A' } };
  ...
  for (var di = 0; di < NR; di++) {
    ...
    V1_CHEM_COLUMNS.forEach(function(cc) {
      ...
      if (val > 0) {
        // Cols 3-5 (gr/buc/kg): integer. Cols 6-10 (L/bidoane): keep up to 2 decimals
  ...
  // ── 4. Clear unused data rows (keep visible — match template layout) ──
  for (var cr = FIRST_DATA_ROW + NR; cr <= FIRST_DATA_ROW + TEMPLATE_SLOTS - 1; cr++) {
    var cRow = ws.getRow(cr);
    for (var cc2 = 1; cc2 <= LAST_COL; cc2++) {
      cRow.getCell(cc2).value = '';
  ...
  return loadExcelJS().then(async function() {
    try {
      ...
      if (typeof TEMPLATE_V1_B64 === 'undefined') {
        throw new Error('Template V1 nu este disponibil. Verifică template-data.js.');
      }
  ...
  return loadExcelJS().then(async function() {
    try {
      ...
      if (typeof TEMPLATE_V1_B64 === 'undefined' || typeof TEMPLATE_V2_B64 === 'undefined') {
        throw new Error('Template-urile nu sunt disponibile. Verifică template-data.js.');
      }
